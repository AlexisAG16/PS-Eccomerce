...
const statusStyles = {
  ...
  COMPLETED: 'bg-brand-bg text-brand-text border-gray-300',
...
const OrdersMyModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-brand-surface text-brand-text rounded-4xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden border border-brand-border animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-b border-brand-border flex justify-between items-center bg-brand-bg">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-brand-secondary/20 rounded-full text-brand-text-muted hover:text-brand-text transition-colors cursor-pointer"
          >
            <FaTimes size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-4 text-[9px] font-black text-brand-text-muted uppercase tracking-widest animate-pulse">Buscando compras...</p>
            </div>
          ) : orders.length > 0 ? (
            orders.map((order) => (
              <div key={order._id} className="border border-brand-border bg-brand-bg p-5 rounded-3xl shadow-sm hover:shadow-md transition-all">

                {/* INFO TOP GENERAL */}
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-brand-border pb-3 mb-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-black text-brand-text bg-brand-surface px-3 py-1 rounded-xl border border-brand-border">
                      #{order._id.slice(-6).toUpperCase()}
                    </span>
                    <div className="flex items-center text-brand-text-muted gap-1 text-xs font-medium">
                      <FaCalendarAlt size={10} />
                      {new Date(order.createdAt).toLocaleDateString()}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-brand-surface text-brand-text-muted border border-brand-border">
                      {orderTypeLabels[order.orderType] || order.orderType}
                    </span>
                    <span className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-md border ${statusStyles[order.status] || 'bg-brand-bg text-brand-text-muted'}`}>
                      {order.status}
                    </span>
                  </div>
                </div>
                    ...
                    <div key={idx} className="flex justify-between items-center text-xs">
                      ...
                      <p className="text-brand-text-muted font-medium">
                        ...
                        {item.productSnapshot?.sku && (
                          <span className="text-[10px] text-brand-text-muted ml-2">
                            ({item.productSnapshot.sku})
                          </span>
                        )}
                      </p>
                      ...
                      <div className="text-right">
                        <p className="font-bold text-brand-text">${item.subtotal}</p>
                        {item.quantity > 1 && (
                ...
                <div className="bg-brand-surface p-4 rounded-2xl flex flex-wrap justify-between items-center gap-4">
                  <div className="text-[11px] text-brand-text-muted font-medium space-y-1">
                    <p>
                      🚚 Entrega: <span className="font-bold text-brand-text">{order.deliveryType === 'PICKUP' ? 'Retiro' : 'Envío'}</span>
                  ...
                  <div className="text-right space-y-1 min-w-[120px]">
                    ...
                    {(order.discount > 0 || order.shippingCost > 0) && (
                      <div className="text-[10px] font-bold text-brand-text-muted uppercase tracking-tight flex justify-between gap-4">
                        <span>Subtotal:</span>
            ...
            <div className="text-center py-16 bg-brand-bg rounded-3xl border-2 border-dashed border-brand-border">
              <p className="text-brand-text-muted font-medium text-sm">No registras ninguna orden en tu cuenta todavía.</p>
