  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="mt-4 text-[10px] font-black text-brand-text-muted uppercase tracking-widest">Cargando ficha...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-black text-brand-text uppercase tracking-tighter">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[10px] text-brand-text-muted">ID: {user._id}</span>
          </div>
        </div>
        <div className="text-right">
          <p className="text-[9px] font-black text-brand-text-muted uppercase">Último Acceso</p>
          <p className="text-xs font-bold text-brand-text">{user.lastLogin ? new Date(user.lastLogin).toLocaleString() : 'Nunca'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-brand-surface border border-brand-border p-6 rounded-3xl shadow-sm">
            <h3 className="text-[10px] font-black text-brand-text-muted uppercase mb-4 flex items-center gap-2">
              <FaShieldAlt /> Rol y Estado
            </h3>
            <div className="space-y-4">
              <div>
                <label className="text-[9px] text-brand-text-muted">Rol del Sistema</label>
                <p className="text-sm font-black text-brand-text uppercase">{user.role?.name || 'Usuario'}</p>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-brand-surface p-6 rounded-3xl border border-brand-border shadow-sm">
            <h2 className="text-[10px] font-black text-brand-text-muted uppercase mb-4 flex items-center gap-2">
              <FaUser /> Información de Contacto
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[8px] uppercase text-brand-text-muted font-bold">Email</label>
                <p className="text-sm font-bold text-brand-text">{user.email}</p>
              </div>
              <div>
                <label className="text-[8px] uppercase text-brand-text-muted font-bold">Teléfono</label>
                <p className="text-sm font-bold text-brand-text">{user.phone || 'No registrado'}</p>
              </div>
            </div>
          </div>
            ...
            {user.address?.location?.coordinates ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-brand-surface p-4 rounded-2xl">
                  <p className="text-[10px] font-black text-brand-text uppercase">{user.address.alias}</p>
                  <p className="text-sm font-bold">{user.address.street} {user.address.number}</p>
                  <p className="text-xs text-brand-text-muted">{user.address.city}</p>
                  {user.address.apartment && <p className="text-[10px] text-brand-text-muted italic">Dpto: {user.address.apartment}</p>}
                </div>
                <div className="h-32 rounded-2xl overflow-hidden border border-brand-border">
                  <MapViewer coords={user.address.location.coordinates} />
                </div>
              </div>
            ) : (
              <p className="text-xs text-brand-text-muted italic">No hay ubicación registrada.</p>
            )}
          </div>
        </div>
